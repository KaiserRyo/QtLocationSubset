...
/*!
    ...
    \ingroup maps
        \headerfile qgeoserviceprovider.cpp <QtLocationSubset/QGeoServiceProvider>
    @xmlonly
    ...
    in the underlying data sets could cause serious incongruencies between
    the services.

    Subclasses of QGeoServiceProvider guarantee that the different services
    that they provide are interoperable.

    Note that BB10 provides a single service provider, and this provider only supports
    the geocoding functionality. Maps are supported by the MapView API.
*/
...
{
    d_ptr->error = QGeoServiceProvider::NotSupportedError;
    d_ptr->errorString = "There is no QtMobilitySubset mappingManager() support.";
    return 0;
}
...
{
    d_ptr->error = QGeoServiceProvider::NotSupportedError;
    d_ptr->errorString = "There is no QtMobilitySubset routingManager() support.";
    return 0;
}
...
{
    QStringList paths;
    paths << mobilityPlugins(QLatin1String("geoservices_subset"));
